...
export function GlobalActions() {
  ...
  return <>
    ...
    {confirmation === 'download' && <div className="modal-backdrop" role="presentation"><section className="modal external-site-modal download-modal" role="dialog" aria-modal="true" aria-labelledby="download-title"><div className="request-icon"><Download size={25} /></div><p className="eyebrow">Aplicativo para Windows</p><h3 id="download-title">Deseja baixar o EntreTelas?</h3><p>A versão portátil abre sem instalação. O instalador é opcional e recebe atualizações automáticas completas.</p><div className="download-options"><button type="button" className="secondary" onClick={() => setConfirmation('')}>Não</button><button type="button" onClick={() => download(PORTABLE_DOWNLOAD_URL)}>Baixar portátil</button><button type="button" onClick={() => download(INSTALLER_DOWNLOAD_URL)}>Baixar instalador</button></div></section></div>}
  </>
}
