...
assert.equal(first.rows[0].encoderImplementation, null)
assert.equal(first.rows[0].powerEfficientEncoder, null)
const hardware = make(1000, 10, 1000, .1)
Object.assign(hardware.get('video'), { encoderImplementation: 'ExternalEncoder', powerEfficientEncoder: true, targetBitrate: 8e6 })
const hardwareRow = summarizeStats(hardware).rows[0]
...
assert.equal(second.rows[0].localCandidateType, 'relay')
assert.equal(second.rows[0].remoteCandidateType, null)
