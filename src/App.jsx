...
function FriendSiteButton() {
  const [confirming, setConfirming] = useState(false)
  const openSite = () => {
    window.open('https://osrsiron.com', '_blank', 'noopener,noreferrer')
    setConfirming(false)
  }
  return <>
    <button type="button" className="friend-site-button" onClick={() => setConfirming(true)}><ExternalLink size={15} />OSRS Iron Tools</button>
    {confirming && <div className="modal-backdrop" role="presentation"><section className="modal external-site-modal" role="dialog" aria-modal="true" aria-labelledby="external-site-title"><div className="request-icon"><ExternalLink size={25} /></div><p className="eyebrow">Link externo</p><h3 id="external-site-title">Abrir OSRS Iron Tools?</h3><p>Você será levado para um site de Tools para irons no OldschoolRunescape, deseja continuar?</p><div className="modal-actions"><button type="button" className="secondary" onClick={() => setConfirming(false)}>Não</button><button type="button" onClick={openSite}>Sim</button></div></section></div>}
  </>
}
