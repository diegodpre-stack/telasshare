...
Ao iniciar uma captura no aplicativo, o seletor mostra as telas e janelas disponíveis. Nada pode iniciar a captura silenciosamente. Tela inteira pode incluir todo o áudio do sistema. Ao selecionar uma janela, o aplicativo força a transmissão sem áudio, pois o Windows/Electron não fornece áudio isolado por processo de forma segura. Essa regra também é validada novamente fora da interface para impedir que Discord ou outros programas vazem. Para compartilhar somente o áudio de um site, use a versão web no Chrome/Edge e selecione uma guia.
