...
export default function MediaDiagnostics({ peers, localStream }) {
  ...
  return <details style={{ margin: '12px 0', padding: 12, border: '1px solid #314759', borderRadius: 10 }}>
    ...
    {runtime && <p>Captura desktop: {runtime.zeroCopyCapture ? 'zero-copy (frame permanece na GPU)' : runtime.zeroCopyDisabledByCrashes ? 'padrão — o zero-copy foi desligado sozinho após o processo de GPU falhar; para tentar de novo, inicie com ENTRETELAS_GPU_CAPTURE=1' : 'padrão (zero-copy desativado nesta máquina)'} · Hardware em baixa resolução: {runtime.lowResolutionHardware ? 'permitido, conforme suporte do codec' : 'política padrão do Chromium'}. O uso real do encoder continua indicado em cada conexão.</p>}
